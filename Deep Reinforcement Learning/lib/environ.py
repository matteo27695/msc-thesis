...
class RFDynamicModel:
    def __init__(self, numba):
        self.numba = numba
        self.model = [OdeModel(a=0.015, alpha=np.random.normal(0.15, 0.005), b=2, gamma=43),
                      OdeModel(a=0.015, alpha=np.random.normal(0.04, 0.0005), b=1, ratio=3)]
        # self.model[0] tumor dynamical evolution
        # self.model[1] normal cell dynamical evolution

    def reset(self):
        self.model[0].reset([1, 0])
        self.model[1].reset([1, 0])
        self.model[0].alpha = np.random.normal(0.15, 0.005)
        self.model[1].alpha = np.random.normal(0.04, 0.0005)
...
class ActionSpace:
    def __init__(self, dose_max, dose_step):
        ...
        self.space = np.linspace(0, dose_max, 11)
...
class Env:
    ...
    def reward(next_state):
        ...
        return (healthy_lastvalue*healthy_lastvalue)-tumor_lastvalue
    ...
    def step(self, action_index):
        ...
        if self.observation.tot_dose > self.tot_dose_max or self.observation.fractions == self.num_fractions or self.observation.tumor_lastvalue < 0.001:
